...
@Component({
  ...
  styleUrls: ['./edit-rfp.component.css'],
})
export class EditRfpComponent {
  files: File[] = [];
  ...
  selectedRfpID = this.storageService?.getSelectedRfpId();
  getUserEditRfpInfo$ = this.apiService
    .getRfpEditInfo(
      this.selectedRfpID(),
    ...
    .pipe(
        ...
        if (results?.selectAdResult?.length < 1)
          ...
          selectAdResult.forEach((item) => {
            if (item?.userRFPImage) {
              const uint8Array = new Uint8Array(item?.userRFPImage.data);
      ...
      switchMap(() =>
        this.apiService.getApplicationSetting().pipe(
          takeUntilDestroyed(this.destroyRef),
          tap((info: any) => {
            this.maxAdvertisementSliderImage = info.maxRFPSliderImage;
  ...
  constructor(
    ...
    if (adObject?.rfpSelected && adObject?.rfpAction === 'edit')
      this.advertisement = this.storageService?.getSelectedRfp()();
    ...
    this.getUserEditRfpInfo$.subscribe();
