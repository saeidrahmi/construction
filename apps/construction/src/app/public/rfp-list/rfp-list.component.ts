...
export class RfpListComponent {
    ...
    this.apiService
      ...
      .pipe(
        ...
        tap((list: any) => {
          ...
          if (this.storageService.getRfpSearchFilters()()?.length > 0)
            this.filterResults();
        })
      )
      .subscribe();
  ...
  remove(filter: string): void {
    const index = this.storageService.getRfpSearchFilters()().indexOf(filter);
    if (index >= 0) {
      // this.filters.splice(index, 1);
      this.announcer.announce(`Removed ${filter}`);
      this.storageService.removeRfpSearchFilters(filter);
      this.filterResults();
    }
  }
  ...
  filterCity(city: string, province: string) {
    ...
    this.storageService.updateRfpSearchFilters(cityStr, province);
  }
  filterCategory(tag: string) {
    const tagStr = 'Category (' + tag + ')';
    this.storageService.updateRfpSearchFilters(tagStr, null);
  }
  clearAllFilters() {
    this.filteredAdvertisements = this.allAdvertisements;
  ...
  filterProvince(province: string) {
    ...
    this.storageService.updateRfpSearchFilters(provinceStr, province);
  }
  filterRating(rate: number) {
    ...
    this.storageService.updateRfpSearchFilters('Rating (' + rating + ')', null);
  }
  filterResults() {
    const filters = this.storageService.getRfpSearchFilters()();
    if (filters?.length > 0) {
